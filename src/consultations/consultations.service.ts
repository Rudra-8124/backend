  ...
  async bookConsultation(
    ...
    try {
      const result = await this.dataSource.transaction(async (manager) => {
        ...
        // 2. Atomic Conditional UPDATE inside manual span 'booking.hold_slot'
        const heldSlot = await withActiveSpan(
          'booking.hold_slot',
          async () => {
        ...
        // 4. Insert Consultation PENDING_PAYMENT inside manual span 'booking.create_consultation'
        return withActiveSpan(
          'booking.create_consultation',
          async () => {
            const consultationId = randomUUID();
            const partitionMonth = heldSlot.partition_month;
            const traceparent =
              incomingTraceparent || getActiveTraceparent() || generateTraceparent();
            ...
            // 5. Insert Transactional Outbox event with propagated trace context
            const outboxPayload = {
              consultationId,
              patientId,
            ...
            await manager.query(
              `INSERT INTO outbox_events
                (event_type, aggregate_id, aggregate_type, payload, status, traceparent, retry_count, attempts, created_at, updated_at)
          ...
          },
          {
            attributes: {
              'booking.patient_id': patientId,
              'booking.doctor_id': heldSlot.doctor_id,
              'booking.slot_id': heldSlot.id,
            },
          },
        );
      });

      this.metricsService?.recordBookingAttempt('success');
      return result;
    } catch (err: unknown) {
      if (err instanceof ConflictException) {
        this.metricsService?.recordBookingAttempt('conflict');
      } else {
        this.metricsService?.recordBookingAttempt('error');
      }
      throw err;
    }
  }
